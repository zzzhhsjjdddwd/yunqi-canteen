...
import { ArrowLeft, Clock, Package, CheckCircle2, XCircle, ChefHat, Home, RefreshCw, CreditCard, XOctagon, MapPin, Trash2 } from 'lucide-react';
...
export default function OrdersPage() {
  ...
  const renderContent = () => {
    ...
    if (id) {
      ...
      return <OrderDetail order={order} onDelete={handleDeleteOrder} />;
    }
        ...
        <DialogContent className="sm:max-w-md rounded-2xl border-white/50 bg-white/90 backdrop-blur-xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-3">
              ...
              <span className="text-lg font-semibold">删除订单</span>
            </DialogTitle>
            <DialogDescription className="text-muted-foreground pt-2">
              确定要删除这个订单吗？删除后订单将在您的订单列表中移除，商家仍可查看此订单。
            </DialogDescription>
          </DialogHeader>
          <div className="flex gap-3 pt-4">
            ...
            <Button
              ...
              className="flex-1 rounded-full bg-gradient-to-r from-red-500 to-red-600 text-white shadow-lg shadow-red-500/30 hover:shadow-xl hover:shadow-red-500/40"
            >
              确认删除
            </Button>
          </div>
...
function OrderCard({ order, onDelete }: { order: Order; onDelete: (id: string) => void }) {
  ...
  const canDelete = ['completed', 'cancelled'].includes(order.status) || order.deletedByUser;
  ...
  return (
    <div className="glass-card overflow-hidden transition-all hover:shadow-xl hover:-translate-y-0.5 duration-200 cursor-pointer group active:scale-[0.99]"
         onClick={() => navigate(`/menu/orders/${order.id}`)}>
          ...
          <div className="flex items-center gap-2">
            {canDelete && (
              <button
                ...
                className="flex h-8 w-8 items-center justify-center rounded-full text-red-500 hover:bg-red-50 transition-all active:scale-95"
                title="删除订单"
              >
          ...
          {canDelete && (
            <div className="stagger-fade-in" style={{ animationDelay: '300ms' }}>
              <button
                ...
                className="w-full rounded-full border border-red-200 bg-red-50 text-red-600 px-4 py-3 font-medium hover:bg-red-100 transition-all flex items-center justify-center gap-2 active:scale-95"
              >
        ...
        <DialogContent className="sm:max-w-md rounded-2xl border-white/50 bg-white/90 backdrop-blur-xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-3">
              ...
              <span className="text-lg font-semibold">取消订单</span>
            </DialogTitle>
            <DialogDescription className="text-muted-foreground pt-2">
